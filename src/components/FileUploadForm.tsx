...
type OmittedProps = Omit<
  ...
  "onChange" | "hidden" | "type" | "id" | "name" | "disabled"
>;

interface ImageUploadProps extends OmittedProps {
  formActionFunction: (
    state: Awaited<unknown | null | undefined>,
    payload: FormData
    ...
    const clipboardListener = async (e: KeyboardEvent) => {
      ...
      if (e.ctrlKey && e.key === "v") {
        try {
          ...
          if (err instanceof Error) {
            console.error(
              "ImageUpload Clipboard Clipboard Event:",
              err.name,
              err.message
            );
          }
  ...
  return (
    <>
      <form
        ...
        onDrop={dropListener}
        className="relative min-w-full mb-4"
        ref={formRef}
        action={formAction}
      >
        {/* Status updates */}
        ...
        <h1 className="text-3xl font-bold">
          {actionState ? actionState : "Choose a file..."}
        </h1>
        ...
        <label
          ...
          className={`${props.className} ${
            isPending
              ? "bg-slate-800 hover:bg-slate-800 z-20 opacity-50 loading-circle"
              : ""
          }`}
        >
          <span
            className={
              isPending
                ? "absolute w-[25px] h-[25px] rounded-full border-5 border-white border-b-transparent inline-block border-box animate-spin"
                : "hidden"
            }
          ></span>
          {`Upload ${alreadyUploaded ? "Another" : ""} File`}
        </label>
        ...
        {hovering && (
          <>
            ...
            <div
              className="z-20 absolute pointer-events-none font-extrabold rounded-md top-0 left-0 
